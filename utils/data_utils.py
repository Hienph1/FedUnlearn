...
def data_set(data_name):
    # download=False: data da duoc download san, tranh loi network khi chay tren cluster.
    # root phai khop voi cau truc thu muc thuc te (tham khao load_datasets.py):
    #   MNIST/FashionMNIST : root='./data'  -> ./data/MNIST/raw/<files>
    #   CIFAR-10/100       : root='./data'  -> ./data/cifar-10-batches-py/<files>
    if data_name == 'mnist':
        ...
        raise ValueError(
            "data_set(): unrecognized data_name={!r}. "
            "Supported: 'mnist', 'fashionmnist', 'cifar10', 'cifar100'".format(data_name))
...
def separate_data(data, num_clients, num_classes, args, niid=False, balance=False, partition=None, class_per_client=None):
        ...
        while min_size < least_samples:
            if try_cnt > 1:
                print(
                    ...
                    'Try allocating again for the {}-th time.'.format(
                        least_samples, try_cnt))
    ...
    for client in range(num_clients):
        print("Client {}\t Size of data: {}\t Labels: {}".format(
            client, len(X[client]), np.unique(y[client])))
        print("\t\t Samples of labels: {}".format(
            [i for i in statistic[client]]))
        print("-" * 50)
...
def split_test_proxy(test_loader, args):
    ...
    proxy_loader = DataLoader(data_proxy, batch_size=args.bs, shuffle=True)
...
def split_data(X, y, args, client_at=None):
    ...
    for i in range(len(y)):
        ...
        test_loaders.append(DataLoader(test_data, batch_size=args.bs, shuffle=True))
